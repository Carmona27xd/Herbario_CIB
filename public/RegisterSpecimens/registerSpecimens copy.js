document.addEventListener('DOMContentLoaded', function() {
    ...
    if (municipioSelect) {
        municipioSelect.addEventListener('change', () => {
            ...
            .then(res => res.json())
            .then(data => {
                // DEPURACIÓN: Abre la consola (F12) y mira qué nombre aparece aquí
                ...
                if (data.length > 0) {
                    data.forEach(localidad => {
                        ...
                        // LÓGICA ROBUSTA: Busca la propiedad correcta sin importar mayúsculas/minúsculas
                        // Busca una clave que empiece con "id" y contenga "loc" (ej. idLocality, id_locality, id)
                        const idKey = Object.keys(localidad).find(key => key.toLowerCase().includes('id') && (key.toLowerCase().includes('loc') || key === 'id'));
                        
                        const idVal = localidad[idKey] || localidad.idLocality; // Valor encontrado o intento directo
                        
                        if (!idVal) console.warn("¡Ojo! No se encontró el ID en este objeto:", localidad);

                        option.value = idVal;
                        option.textContent = localidad.name || localidad.nombre; // Intenta 'name' o 'nombre'
