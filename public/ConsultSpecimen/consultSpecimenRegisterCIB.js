document.addEventListener('DOMContentLoaded', () => {
    const inputRegistroCIB = document.getElementById('registroCIBInput');
    ...
    fetch('../../backend/ConsultSpecimens/servicesFetchConsultSpecimens.php')
        ...
        .catch(err => console.error('Error al obtener ejemplares:', err));

    // Filtro dinámico por Registro CIB
    inputRegistroCIB.addEventListener('input', () => {
        const texto = inputRegistroCIB.value.toLowerCase();
        const filtrados = ejemplaresOriginal.filter(ejemplar =>
            ejemplar.specimenImage.toLowerCase().includes(texto)
        );
    ...
    function actualizarEstadoBotones() {
        ...
        btnMostrar.disabled = checkboxes.length === 0;
        btnDescargar.disabled = checkboxes.length === 0;
    }

    //Ingaturroña y yo que pensaba que reutilizar seria mas facil
